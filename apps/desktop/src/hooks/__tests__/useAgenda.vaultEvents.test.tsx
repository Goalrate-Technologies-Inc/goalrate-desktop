...
describe("useAgenda vault events", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(listen).mockResolvedValue(() => {});
    vi.mocked(emit).mockResolvedValue(undefined);
    vi.mocked(agendaIpc.getAgendaWarnings).mockResolvedValue([]);
    vi.mocked(agendaIpc.getTaskMetadata).mockResolvedValue({});
    vi.mocked(agendaIpc.getOutcomes).mockResolvedValue([]);
    vi.mocked(agendaIpc.getChatHistory).mockResolvedValue([]);
    vi.mocked(agendaIpc.getCheckIn).mockResolvedValue(null);
    vi.mocked(agendaIpc.getRecentStats).mockResolvedValue([]);
  });
  ...
  it("refreshes agenda data when the active vault changes on disk", async () => {
    ...
    expect(agendaIpc.getPlan).toHaveBeenCalledTimes(1);
    ...
    expect(agendaIpc.getPlan).toHaveBeenCalledTimes(2);
  });
  ...
  it("coalesces watcher refreshes while a refresh is already running", async () => {
    ...
    await waitFor(() => {
      expect(agendaIpc.getPlan).toHaveBeenCalledTimes(3);
    });
